...
def run_test(test: str, race: bool, timing: bool, timeout: str):
    test_cmd = ["go", "test", f"-run={test}"]
    if race:
        test_cmd.append("-race")
    if timeout:
        test_cmd.append("-timeout="+timeout)
    if timing:
        test_cmd = ["time"] + cmd
...
def run_tests(
    ...
    timeout: str           = typer.Option('',     '--timeout',         '-T',    help='Maximum time a test can last')
    # fmt: on
):
